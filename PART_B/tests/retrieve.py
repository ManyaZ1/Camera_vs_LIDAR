...
def refine_inliers_by_distance(points: np.ndarray, plane_model, inlier_indices):
    ...
    refined = [i for i in inlier_indices if dists[i] < 0.08]  # αυστηρό όριο
    return refined
...
def process_file(bin_path: Path, dist: float, iters: int, img_dir: Path, calib_dir: Path):
    ...
    # EXTRA: Κράτα μόνο το κεντρικό τμήμα του δρόμου (Y ~ πλάτος)
    road_points = np.asarray(road.points)
    mask = np.abs(road_points[:, 1]) < 4.0  # ±4 μέτρα από το κέντρο
    ...
    print(f"[INFO] Road points: {len(inliers)} / {len(pcd.points)}")
