...
def draw_projected_box(img, box: o3d.geometry.OrientedBoundingBox, P):
    pts = np.asarray(box.get_box_points())  # 8 corners
    h = np.hstack([pts, np.ones((8, 1))])
    uvw = (P @ h.T).T
    z = uvw[:, 2]
    
    if not np.all(z > 0):
        return  # skip this box entirely if any corner is behind the camera
...
def process_frame_new(bin_path, args):
    ...
    # --- obstacle clustering ---------------------------------------
    non_ground = np.asarray(pcd.points)[list(set(range(len(pcd.points))) - set(all_inliers))]
    cloud_fg   = o3d.geometry.PointCloud()
    cloud_fg.points = o3d.utility.Vector3dVector(non_ground)

    labels = np.array(
        cloud_fg.cluster_dbscan(eps=0.6, min_points=20, print_progress=False)
    )

    clusters = [non_ground[labels == k] for k in np.unique(labels) if k >= 0]
    obboxes  = [o3d.geometry.OrientedBoundingBox.create_from_points(
                o3d.utility.Vector3dVector(cl)) for cl in clusters]
    obboxes = [b for b in obboxes if np.all(b.extent < 10) and np.all(b.extent > 0.2)]

    print(f"{len(clusters)} obstacle clusters")

    # draw in Open3D if you want
    cloud_fg.paint_uniform_color([0.2, 0.2, 0.2])
    o3d.visualization.draw_geometries([cloud_fg, *obboxes])
...
# inside process_frame_new() after `img = cv2.imread(...)`
    for box in obboxes:
        draw_projected_box(img, box, proj)

    cv2.imshow("Road + Obstacles", img)
    cv2.waitKey(0)
