...
def piecewise_ransac(pcd, dist, iters, max_planes=2, min_pts=500):
    remaining = np.asarray(pcd.points)
    planes = []

    for _ in range(max_planes):
        pc = pc_to_o3d(remaining).voxel_down_sample(0.1)
        if len(pc.points) < min_pts:
            break

        plane, inliers = pc.segment_plane(distance_threshold=dist, ransac_n=3, num_iterations=iters)
        normal = np.array(plane[:3])
        if not is_valid_plane_normal(normal):
            continue  # skip tilted planes

        refined = refine_inliers_by_distance(np.asarray(pc.points), plane, inliers)
        if len(refined) == 0:
            continue

        planes.append((plane, np.asarray(pc.points)[refined]))
...
def process_frame(bin_path, args):
    ...
    all_planes = piecewise_ransac(pcd, args.dist, args.iters)
    road_pts = np.vstack([pts for _, pts in all_planes]) if all_planes else np.empty((0, 3))
